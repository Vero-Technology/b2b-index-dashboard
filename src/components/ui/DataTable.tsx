        ...
        <table className="w-full text-left text-sm">
            ...
            <tr className="border-b border-surface-700">
              {columns.map((col) => (
                <th
                  key={col.key}
                  className={cn(
                    'px-4 py-2.5 text-xs font-medium uppercase tracking-wider text-gray-400',
                    col.className
                  )}
                >
                  {col.header}
          ...
          <tbody>
            {data.map((row, i) => (
              <tr
                key={i}
                className="border-b border-surface-800/50 transition-colors hover:bg-surface-950"
              >
                {columns.map((col) => (
                  <td
                    key={col.key}
        ...
        <div className="flex items-center justify-between border-t border-surface-700 px-4 py-3">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              disabled={page <= 1}
              className="rounded p-1 text-gray-400 transition-colors hover:bg-surface-800 hover:text-gray-700 disabled:opacity-30 disabled:pointer-events-none"
            >
              <ChevronLeft size={16} />
            </button>
